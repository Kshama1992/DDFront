...
import SpaceFilterComponent from '@pages/space-list/space-filter.component';
...
interface HeaderSpaceListProps {
	venueName?: string;
...
function HeaderSpaceList({ onChange, venueName, defaultValues, onFilterInitiated }: HeaderSpaceListProps) {
	...
	return (
		<Paper className={classes.root}>
			<Grid container direction="row" justifyContent="space-between" alignItems="center">
				...
				{!isAdmin() && (
					<Grid sx={{ display: { xs: 'none', md: 'block' } }} item xs={12} sm={authBody ? 6 : 5} md={authBody ? 8 : 5}>
						<Grid container justifyContent="center" alignItems="center">
							{authBody && (
								<Grid item sm={2.5} xs={5}>
									{subsDropDown()}
								</Grid>
							)}
							{authBody && (
							...
							<Grid item sm={authBody ? 7 : 11} xs={10}>
								<SpaceFilterComponent
									onFilterInitiated={onFilterInitiated}
									venueName={venueName}
...
export default HeaderSpaceList;
